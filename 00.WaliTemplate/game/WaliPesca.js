					...
					case HookState.ROLL_DOWN:
						...
						for (let i = 0; i < fishes.length; i++) {
							if (fishes[i].isHooked(this.hook)) {
								...
								const x = fishes[i].direction === 1 ? -200 : screenWidth + 200;
								fishes[i] = new Fish(x, fishes[i].y, type, fishes[i].direction);
...
class Hook extends ColliderObject {
		...
		switch (this.state) {
			case HookState.ROLL_UP:
				...
				if (this.isDown()) {
					...
					// The bigge the fish, the slower the movement
				...
				if ((game.inputs.mouse.isMouseDown() || game.inputs.keypad.isKeyDown(game.utils.keycode.space) || game.inputs.keypad.isKeyDown(game.utils.keycode.down))) {
					if (this.bottom.y > MIN_HEIGHT_HOOK - this.speed / 2) this.bottom.y -= this.speed / 2;
				}
				else if (this.bottom.y < MAX_HEIGHT_HOOK) this.bottom.y += this.speed;*/
	...
	getCollider() {
		return new Rect(this.bottom.x - 6, this.bottom.y - 6, 12, 12);
	}
...
class Fish extends ColliderObject {
	...
	update() {
		...
		// Bounds
		if (this.direction === 1 && this.pos.x > screenWidth + 200) this.pos.x = -200;
		else if (this.direction === -1 && this.pos.x < -200) this.pos.x = screenWidth + 200;
...
//#region METHODS
// Fishes
const typesList = [1, 2, 0, 3, 1, 0, 2, 3, 2, 0, 1, 3, 2, 0, 1, 3, 2, 0, 1, 3, 0, 1, 2, 3, 1, 2, 0, 3, 1, 2, 0, 3, 0, 2, 1, 3, 1, 0, 2, 3, 0, 1, 2, 3, 1, 2, 0, 3, 2, 0, 1, 3, 0, 2, 1, 3, 2, 0, 1, 3, 0, 1, 2, 3, 0, 2, 1, 3, 1, 2, 0, 3, 0, 1, 2, 3, 0, 2, 1, 3, 1, 0, 2, 3, 2, 0, 1, 3, 1, 2, 0, 3, 0, 1, 2, 3, 1, 2, 0, 3, 0, 2, 1, 3, 1, 2, 0, 3, 1, 0, 2, 3, 0, 2, 1, 3, 1, 2, 0, 3, 1, 2, 0, 3, 1, 0, 2, 3, 1, 0, 2, 3, 0, 1, 2, 3, 0, 2, 1, 3, 1, 2, 0, 3, 1, 2, 0, 3, 2, 1, 0, 3, 2, 0, 1, 3, 1, 2, 0, 3, 1, 2, 0, 3, 2, 1, 0, 3, 0, 1, 2, 3, 1, 2, 0, 3, 2, 1, 0, 3, 1, 2, 0, 3, 2, 1, 0, 3, 2, 1, 0, 3, 1, 0, 2, 3, 1, 0, 2, 3, 0, 1, 2, 3, 2, 1, 0, 3, 0, 2, 1, 3, 1, 0, 2, 3, 0, 1, 2, 3, 0, 2, 1, 3, 0, 1, 2, 3, 2, 0, 1, 3, 0, 2, 1, 3, 0, 1, 2, 3, 0, 2, 1, 3, 0, 1, 2, 3, 2, 0, 1, 3, 1, 0, 2, 3, 1, 0, 2, 3, 2, 0, 1, 3, 2, 1, 0, 3, 1, 2, 0, 3, 0, 2, 1, 3, 0, 1, 2, 3, 1, 2, 0, 3, 1, 2, 0, 3, 1, 2, 0, 3, 1, 2, 0, 3, 0, 2, 1, 3, 0, 2, 1, 3, 2, 1, 0, 3, 1, 2, 0, 3, 0, 2, 1, 3, 0, 2, 1, 3, 2, 0, 1, 3, 2, 0, 1, 3, 2, 1, 0, 3, 0, 2, 1, 3, 1, 2, 0, 3, 2, 1, 0, 3, 0, 1, 2, 3, 2, 1, 0, 3, 2, 1, 0, 3, 0, 2, 1, 3, 2, 0, 1, 3, 0, 1, 2, 3, 0, 1, 2, 3, 1, 0, 2, 3, 2, 0, 1, 3, 1, 2, 0, 3, 1, 2, 0, 3, 1, 0, 2, 3, 1, 2, 0, 3, 1, 0, 2, 3];
let typePointer = 0;
const getFishType = () => {
	typePointer = typePointer.overflow(typesList.length);
	switch (typesList[typePointer++]) {
		default:
